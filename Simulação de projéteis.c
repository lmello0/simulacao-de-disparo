...
int main(){
	...
	do{ //Apresentacao
		printf("Simulador de lancamento de projeteis\n\n\tby Lucas Mello dos Santos\n\n");
	...
	do{	//Menu
		...
		switch(menu){
			case 1:
				...
				do{ //Angulo
					printf("\tAngulo de inclinacao (entre 0 e 90): ");
					scanf("%f", &inclinacao);
				}while(inclinacao < 0 || inclinacao > 90);
				do{ //Distancia
					printf("\tDistancia do alvo (em metros): ");
					scanf("%f", &distancia);
				}while(distancia < 0);
				do{ //Tamanho
				...
				if(alcanceHorizontal > distancia){
					if(alcanceHorizontal > distancia && alcanceHorizontal < distancia+tamanho){
						...
						printf("O projetil caiu depois do alvo!\n\n");
					}
				} else {
					printf("O projetil caiu antes do alvo!\n\n");
				}
			...
			case 2:
				...
				printf("\tMedia do alcance horizontal: %.2f\n", media/divisaoMedia);
